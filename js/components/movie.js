import { addData, getDataById, deleteDataById, getData, addCommentData } from "./database";
...
export class Movie {
    plotMovie = () => {
        ...
        for (const [key, data] of movieDataList) {
            ...
            const elementAddComment = document.getElementById(`add_comment_${imdbID}`);
            elementAddComment.addEventListener("click", (e) => {
                let elementId = elementAddComment.getAttribute("data_id");
                addMovieComment(elementId);
            });

            const elementViewComment = document.getElementById(`view_comment_${imdbID}`);
...
async function viewMovieComment(id) {
    ...
    if (isFavData === true) {
        ...
        console.log("view movie comment for movie : " + id + ", comment_data:" + comment_data);
    } else {
...
async function addMovieComment(id) {
    ...
    if (isFavData === true) {
        ...
        console.log("add movie comment for movie : " + id + ", comment_data:" + comment_data);
    } else {
